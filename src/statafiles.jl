    ...
    for dfrow in eachrow(outdf)
        for (i,v) in enumerate(dfrow)
            ...
            elseif datatypes[i] == Date
                write(iobuf, datatypes[i](ismissing(v) ? missingval[typelist[i]] : Dates.value(v - Date(1960,1,1))))
            elseif datatypes[i] == DateTime
                write(iobuf, datatypes[i](ismissing(v) ? missingval[typelist[i]] : Dates.value(v - DateTime(1960,1,1))))
            else
